...
engine = create_engine('sqlite:///category_item_user.db?check_same_thread=False')
...
def CategorysItemsJSON():
    ...
    return jsonify(Category=[r.serialize for r in categories],Item=[i.serialize for i in items])
...
@app.route('/catalog/<string:games_name>/<int:games_category_id>/edit/', methods=['GET', 'POST'])
...
# Delete a Category
@app.route('/catalog/<string:games_name>/<int:games_category_id>/delete/', methods=['GET', 'POST'])
def deleteCategory(games_name,games_category_id):
    ...
    if request.method == 'POST':
        ...
        session.commit()
        return redirect(url_for('showCategorys', games_category_id=games_category_id))
    else:
...
def showMenu(games_category_id):
    ...
    if 'username' not in login_session or creator.id != login_session['user_id']:
        return render_template('publicCategoryItem.html', items=items, Category=Categorys, creator=creator,rows_count=rows_num)
    else:
        return render_template('CategoryItem.html', items=items, Category=Categorys, creator=creator,rows_count=rows_num)
...
def ShowItemDescription(games_name, item_name):
    ...
    items = session.query(CategoryItem).filter_by(
        name=item_name).one()
    if 'username' not in login_session or creator.id != login_session['user_id']:
        return render_template('Item_Description.html', games_name=games_name, item_name=item_name,items=items)
    else:
        return render_template('User_Item_Description.html', Category=Categorys, games_name=games_name, item_name=item_name,items=items)
...
def newCategoryItem(games_name):
    ...
    if request.method == 'POST':
        if request.form['name']:
            newItem = CategoryItem(name=request.form['name'], description=request.form['description'], games_category_id=Categorys.id, user_id=Categorys.user_id)
            session.add(newItem)
            session.commit()
            flash('New Menu %s Item Successfully Created' % (newItem.name))
            return redirect(url_for('showMenu', games_category_id=Categorys.id))
    else:
        return render_template('newCategoryItem.html', games_name=games_name)

# Edit a menu item


@app.route('/catalog/<string:games_category_name>/<string:games_item_name>/edit', methods=['GET', 'POST'])
def editCategoryItem(games_category_name, games_item_name):
    if 'username' not in login_session:
        return redirect('/login')
    all_Categorys = session.query(Category).order_by(asc(Category.name))
    editedItem = session.query(CategoryItem).filter_by(name=games_item_name).one()
    Categorys = session.query(Category).filter_by(name=games_category_name).one()
    if login_session['user_id'] != Categorys.user_id:
    ...
    if request.method == 'POST':
        ...
        if request.form['Category_id']:
            editedItem.games_category_id = request.form['Category_id']
        session.add(editedItem)
        session.commit()
        flash('Menu Item Successfully Edited')
        return redirect(url_for('showMenu', games_category_id=Categorys.id))
    else:
        return render_template('editCategoryItem.html', games_category_name=games_category_name, games_item_name=games_item_name, item=editedItem,Categorys=all_Categorys)


# Delete a menu item
@app.route('/catalog/<string:games_category_name>/<string:games_item_name>/delete', methods=['GET', 'POST'])
def deleteCategoryItem(games_category_name, games_item_name):
    if 'username' not in login_session:
        return redirect('/login')
    Categorys = session.query(Category).filter_by(name=games_category_name).one()
    itemToDelete = session.query(CategoryItem).filter_by(name=games_item_name).one()
    if login_session['user_id'] != Categorys.user_id:
